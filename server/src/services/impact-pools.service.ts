...
function mapPool(row: Record<string, unknown>): ImpactPoolSummary {
  return {
    id: String(row.id),
...
class ImpactPoolsService {
  async createPool(input?: { targetActions?: number; title?: string }) {
    ...
    const candidates = await query<{
      ...
      `SELECT a.id, a.quantity, a.location, v.confidence
       FROM actions a
    ...
    const avgConfidence = Math.round(
      candidates.rows.reduce((sum, row) => sum + Number(row.confidence), 0) / candidates.rows.length,
    );
    const geoCount = new Set(
      candidates.rows.map((row) => String(row.location).trim().toLowerCase()),
    ).size;
    ...
    try {
      await withTransaction(async (client) => {
        await client.query(
          `INSERT INTO impact_pools (
             id, title, status, total_actions, total_quantity, avg_confidence, geo_count, pool_hash, created_at
           ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
          [poolId, title, "ready", actionIds.length, totalQuantity, avgConfidence, geoCount, poolHash, createdAt],
        );

        for (const actionId of actionIds) {
          await client.query(
            `INSERT INTO impact_pool_actions (pool_id, action_id, created_at)
             VALUES ($1, $2, NOW())`,
            [poolId, actionId],
          );
        }
      ...
      throw new HttpError(
        500,
        `Impact pool creation failed. ${(error as Error).message ?? "Unexpected database error"}`,
      );
    }
  ...
  async listPools() {
    ...
    return result.rows.map((row) => mapPool(row as Record<string, unknown>));
  }

  async getPoolById(poolId: string): Promise<ImpactPoolDetail> {
    ...
    const pool = mapPool(poolResult.rows[0] as Record<string, unknown>);
  ...
  async exportPoolProof(poolId: string) {
