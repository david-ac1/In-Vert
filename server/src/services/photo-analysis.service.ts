...
import { locationService, type GeocodeResult } from "./location.service.js";
...
class PhotoAnalysisService {
  ...
  async analyze(photoUrl: string, locationText: string): Promise<PhotoAnalysisResult> {
    ...
    if (localPath) {
      try {
        ...
        const exif = (await exifr.parse(localPath).catch(() => null)) as {
    ...
    let claimed: GeocodeResult | null = null;
    let locationDistanceKm: number | null = null;
    if (locationText.trim()) {
      claimed = await locationService.geocode(locationText.trim());
      if (claimed && exifLatitude !== null && exifLongitude !== null) {
        locationDistanceKm = locationService.distanceKm(
  ...
  private inferSourceKind(photoUrl: string): PhotoAnalysisResult["sourceKind"] {
    if (!photoUrl.trim()) return "unknown";
  ...
  private computeStockRisk(photoUrl: string): { score: number; signals: string[] } {
    ...
    try {
      ...
      if (
        pathName.includes("shutterstock") ||
  ...
  private resolveLocalUploadPath(photoUrl: string): string | null {
