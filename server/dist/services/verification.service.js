...
export class VerificationService {
    async verify(action) {
        ...
        const duplicateAction = env.ENABLE_IMAGE_HASH_DUPLICATE_CHECK && media.imageHash
        ...
        const checks = [
            ...
            },
            {
                ...
                detail: action.quantity > 0 && action.quantity <= 10000
        ...
        if (env.ENABLE_LOCATION_MATCH_CHECK) {
            const locationMatched = media.locationDistanceKm === null
                ? true
                : media.locationDistanceKm <= env.LOCATION_MATCH_MAX_KM;
            checks.push({
                name: "location_exif_distance_match",
                passed: locationMatched,
                score: media.locationDistanceKm === null
                    ? 60
                    : locationMatched
                        ? 100
                        : 0,
                detail: media.locationDistanceKm === null
        ...
        if (env.ENABLE_STOCK_PHOTO_CHECK) {
            ...
            checks.push({
                ...
                detail: media.stockSignals.length > 0
        ...
        const hasHardReject = checks.some((check) => !check.passed && hardRejectChecks.has(check.name));
        const approved = !hasHardReject && reasonCodes.length === 0;
        const confidence = Math.round(checks.reduce((acc, check) => acc + check.score, 0) / checks.length);
        return {
