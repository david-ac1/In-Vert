...
import { Client, ContractCallQuery, ContractExecuteTransaction, ContractFunctionParameters, ContractId, Hbar, PrivateKey, TokenCreateTransaction, TokenSupplyType, TokenType, TokenId, TopicCreateTransaction, TopicId, TopicMessageSubmitTransaction, TransferTransaction, } from "@hashgraph/sdk";
...
export class HederaService {
     ...
     */
    async executeTransaction(tx, client) {
        if (awsKmsService.isConfigured() && env.AWS_KMS_KEY_ID) {
    ...
    async getRewardTokenId(client) {
        ...
        const supplyKey = !awsKmsService.isConfigured() && env.HEDERA_PRIVATE_KEY
     ...
     */
    async registerAttestationOnChain(actionId, proofHash) {
        ...
        const response = await new ContractExecuteTransaction()
            ...
            .setFunction("registerAttestation", new ContractFunctionParameters().addString(actionId).addBytes32(proofHashBytes))
            .execute(client);
     ...
     */
    async getOnChainAttestation(actionId) {
        const contractId = await this.getContractId();
        if (!contractId)
            return null;
        const client = await this.getClient();
        const result = await new ContractCallQuery()
            .setContractId(contractId)
            .setGas(50_000)
            .setFunction("getAttestation", new ContractFunctionParameters().addString(actionId))
            .execute(client);
        const exists = result.getBool(2);
        if (!exists)
            return { proofHash: "", timestamp: 0, exists: false };
